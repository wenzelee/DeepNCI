...
def get_batch_data(data, batch, batch_size):
    ...
    return batch_cube_xs,  batch_ys


cube_x = tf.placeholder(tf.float32, [None, 2259404]) # 137 * 133 * 124 * 1
y = tf.placeholder(tf.float32, [None, 1])


x_data = tf.reshape(cube_x, [-1, 137, 133, 124, 1])  
h_pool0 = max_pool_3d(x_data) # 69, 67, 62
...
# initial the first connected layer weight
...
with tf.Session( ) as sess:
    ...
    saver = tf.train.Saver(max_to_keep=0)   # default max_to_keep=5
    
    min_loss = 9
    # training 
    for epoch in range(300):
        for batch in range(n_batch):
            batch_cube_xs, batch_ys = get_batch_data('train', batch, batch_size)
            sess.run(train_step, feed_dict={cube_x:batch_cube_xs, y:batch_ys, keep_prob:0.7})
        
        
        for batch in range(n_batch):
            batch_cube_xs,  batch_ys = get_batch_data('train', batch, batch_size)
            if batch == 0:
                train_prediction_list = sess.run(prediction, feed_dict={cube_x:batch_cube_xs,  keep_prob:1.0})
            else:
                temp = sess.run(prediction, feed_dict={cube_x:batch_cube_xs,  keep_prob:1.0})
        ...
        for batch in range(test_n_batch):
            batch_cube_xs, batch_ys = get_batch_data('test', batch, batch_size)
            if batch == 0:
                test_prediction_list = sess.run(prediction, feed_dict={cube_x:batch_cube_xs, keep_prob:1.0})
            else:
                temp = sess.run(prediction, feed_dict={cube_x:batch_cube_xs, keep_prob:1.0})
       ...
       # estimate the model
        for batch in range(valid_n_batch):
            batch_cube_xs,  batch_ys = get_batch_data('valid', batch, batch_size)
            if batch == 0:
                valid_prediction_list = sess.run(prediction, feed_dict={cube_x:batch_cube_xs,  keep_prob:1.0})
            else:
                temp = sess.run(prediction, feed_dict={cube_x:batch_cube_xs, keep_prob:1.0})
     ...
     # saving model
        saver.save(sess, 'ckpt_cube/tf_Conv3D_cube.ckpt', global_step=epoch + 1)
...
plt.savefig("result/cube/fig_cube.jpg")
